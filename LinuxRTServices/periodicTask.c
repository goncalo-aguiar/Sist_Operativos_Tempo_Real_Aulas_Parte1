...
void * Thread_1_code(void *arg)
{
	...
	if (periodo != 0 ){
		tp.tv_nsec = periodo *  100*100*100;
		tp.tv_sec = 0;	
	}
	else{
...
int main(int argc, char *argv[])
{
	...
	/* Process input args */
	if(argc == 4) {
		if(atoi(argv[2])>99 || atoi(argv[2])<1){
			printf("Usage: priority must be [1,99], now is %s\n\r", argv[2]);
			return -1;
		}
		if(atoi(argv[3])>500 || atoi(argv[3])<50){
			printf("Usage: periodicity must be [50,500], now is %s\n\r", argv[3]);
			return -1;
		}
	}
